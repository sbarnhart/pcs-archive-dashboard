...
| Opening Qty | Current on-hand quantity, with fallback `1` only after a complete stock extract confirms no quantity record | User confirmed Senmox can display negative stock while Embed blocks processing below zero. The available stock extract is capped at 1,000 rows, so unmatched products stay blank; explicit source zeros stay `0` pending confirmation. Put opening quantity only on the first row of a multi-barcode product to prevent double-counting. |
